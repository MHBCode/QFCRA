...
export class LogformService {
  ...
  getDocListByFirmDocType(firmId: number, docTypeID: string, objectID: number) {
    const url = `${this.baseUrlLogForm}get_doc_list_by_firm_doc_type?firmId=${firmId}&docTypeIDs=${docTypeID}&objectID=${objectID}`;
    return this.http.get<any>(url);
  }
